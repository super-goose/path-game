...
const { start, firstTarget, secondTarget, thirdTarget, finalTarget } = getInitialData();
...
test('it gets the next coordinate based on tile, entry, and current coordinate', () => {
  ...
  expect(getNextCoord(finalTarget, 1, {x: 1, y: 1})).toEqual({x: 0, y: 1})
  expect(getNextCoord(finalTarget, 2, {x: 2, y: 2})).toEqual({x: 2, y: 3})
  expect(getNextCoord(finalTarget, 3, {x: 3, y: 3})).toEqual({x: 3, y: 4})
  expect(getNextCoord(finalTarget, 4, {x: 4, y: 4})).toEqual({x: 5, y: 4})
  expect(getNextCoord(finalTarget, 5, {x: 5, y: 5})).toEqual({x: 6, y: 5})
